...
// Discipline CTA (Phase 9).
// Restrained dark band aligned to the footer treatment. The primary action
// preserves the existing request flow: onRequest(topic) sets the requested
// service and routes to /request. Gold is used only as a subtle accent for
// Legal & Evidence, consistent with the homepage discipline system; every other
// discipline uses the red accent.
export default function DisciplineCta({ discipline, onRequest }) {
  ...
  const requestTopic = `${discipline.name} support`;
  const mailHref = `mailto:${siteConfig.email}?subject=${encodeURIComponent(
    `${discipline.name} enquiry`
  )}`;
        ...
        <div className="bg-image-dark px-6 py-12 lg:px-12 lg:py-14">
          <div className="grid gap-8 lg:grid-cols-[1.4fr_1fr] lg:items-end lg:gap-14">
            ...
            <div className="flex flex-col gap-4 sm:flex-row lg:flex-col lg:items-end">
              <button
                type="button"
                onClick={() => onRequest(requestTopic)}
